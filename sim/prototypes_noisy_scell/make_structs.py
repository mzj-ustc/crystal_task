...
def saveXYZ(fxyz, xyz):
    with open(fxyz, "w") as fid:
        fid.write(f"{xyz['lattices'][0,0]} {xyz['lattices'][0,1]} {xyz['lattices'][0,2]}\n")
        fid.write(f"{xyz['lattices'][1,0]} {xyz['lattices'][1,1]} {xyz['lattices'][1,2]}\n")
        fid.write(f"{xyz['lattices'][2,0]} {xyz['lattices'][2,1]} {xyz['lattices'][2,2]}\n")
        fid.write(str(xyz['na'])+"\n")
        for ii in range(xyz['na']):
            site = xyz['sites'][ii]
            fid.write(f"{site[0]} {site[1]} {site[2]} {xyz['an'][ii]} {ii} {ii}\n")
...
def cart2frac(xyz, cart_vecs):
    '''
    Convert Cartesian coordinates to fractional coordinates.
    '''

    return cart_vecs @ np.linalg.inv(xyz['lattices'])
