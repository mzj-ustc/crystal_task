...
################################################################
# Calculate averaged Steinhardt Parameters for all atoms inside an interior sphere.
...
def cal_steinhardt_parameters(l, sites):
    ...
    for m in range(2*l+1):
        qls = 0
        for site in rc_sites:
            qls += cal_q(m-l, l, sites, site)
        qls /= natom
        ql += np.abs(qls)**2
        
    return np.sqrt(4*np.pi/(2*l+1) * ql)
...
if __name__ == '__main__':
    ...
    for l in [2, 4, 6, 8, 10]:
        print(l, cal_steinhardt_parameters(l, structure))
